...
export default async function handler(
  ...
  try {
    ...
    for (const fileId of fileIds) {
      ...
      // Find the highest ancestor folder for the current file
    ...
    const emptyHashes = await prisma.fileHash.findMany({
      where: {
        size: { gte: 0 }, // Filter for non-empty file hashes
        files: { none: {} } // Filter for fileHashes with no associated files
      }
    });
