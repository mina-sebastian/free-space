...
const FileCard: React.FC<FileCardProps> = ({ itemId, itemType, name, link = '', onShare, onMenuClick }) => {
  ...
  return (
    <Paper sx={{ width: '100%', height: 50, p: 2, m: 1, display: 'flex', alignItems: 'center' }}>
      <Typography
        component={Link}
        href={itemType === 'folder' ? `${router.asPath}/${name}` : link}
        variant="h6"
        sx={{ flexGrow: 1, ml: 3 }}
      >
        {name}
      </Typography>
      <IconButton aria-label="more" aria-controls="menu" aria-haspopup="true" onClick={(event) => onMenuClick(event, itemId, itemType, name)} size="large">
        <MoreVertIcon />
      </IconButton>
    </Paper>
  );
